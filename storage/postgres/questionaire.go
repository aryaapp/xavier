...
func (db *QuestionaireDatabase) All(userID int) ([]storage.Questionaire, error) {
	questionaires := []storage.Questionaire{}
	err := db.Select(&questionaires, questionairesAll, userID)
	return questionaires, err
}

func (db *QuestionaireDatabase) Find(uuid string, userID int) (*storage.Questionaire, error) {
